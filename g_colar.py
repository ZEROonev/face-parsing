...
def colorpng(path,filename):
      ...
      '''
      files = list(sorted(os.listdir(folder_base)))
      fnae=[]
      for i,data in enumerate(files):
        fv=list(sorted(os.listdir(os.path.join(folder_base,data))))
        fnae+=fv
      filename=os.path.join(folder_base,fv[k].split('_'),fv[k])
      '''
      filenames=os.listdir(path)
      for k in range(len(filenames)):
          filename=os.path.join(path,filenames[k])
