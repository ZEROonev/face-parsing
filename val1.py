...
import cv2 as cv
#define the config
from utils import vis_parsing_maps
...
val_dataloader = DataLoader(FaceDataset(path,typel='val'), batch_size=1, num_workers=1)
model=ACSPNet()
    ...
    for i, data in enumerate(val_dataloader, 0):
        ...
        with torch.no_grad():
            ...
            if isinstance(outputs, list):
                ...
                im=os.path.join('../cvpr/val/image',name.split('_')[0],name+'.jpg')
                img=cv.imread(im)
                ...
                if i%6==0:
                  #name=str(eval(str(name).lstrip('(').rstrip(')').rstrip(',')))
                  fusing.save("./result/renders/%s.png"%(name))
...
def colorpng(path,filename):

      
      folder_base = './seg_test'
      folder_save = './color'
