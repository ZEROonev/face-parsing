...
def focal_loss(pred,label):
    ...
    for i in range(18):
      pos_num += [pred]
    sumx=sum(pos_num).item()
    weights=1/sumx*pos_num
    loss = 0

    preds = F.interpolate(input=pred, size=(h, w),mode='bilinear', align_corners=True)
    loss += F.cross_entropy(preds, label, ignore_index=255)
    return loss
        ...
        if isinstance(preds_parsing, list):
            for idx, pred_parsing in enumerate(preds_parsing):
                ...
                if idx == len(preds_parsing) - 1:  # Is that the last term ?
                    loss += (torch.mul(self.weighted_criterion(scale_pred, target), torch.where(
                        target == 0, torch.Tensor([1]).cuda(), torch.Tensor([1 + self.alpha]).cuda()))).mean()
                else:
