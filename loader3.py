...
class FaceDataset(Dataset):
  ...
  def __getitem__(self,idx):
    ...
    if self.typel=='train' or self.typel=='val':
      person_id=self.total_name[idx]
      person_face=os.path.join(self.img_path,str(person_id.split('_')[0]))# return the folder name within the folder
      img_frame=os.path.join(person_face,person_id)
      name=str(person_id.split('.')[0])
      image=Image.open(img_frame)
    
      mask_pat=os.path.join(self.seg_path,str(person_id.split('_')[0]))
      mask_path=os.path.join(mask_pat,str(person_id.split('.')[0])+'.png')
